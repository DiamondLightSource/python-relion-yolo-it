            ...
            if line.startswith('#') or len(line) == 0:
                ...
                if equals_index > 0:
                    ...
                    if option_name in option_names:
                        print( '{} = {}'.format(option_name, repr(getattr(self, option_name))), file=out_file)
...
class RelionItGui(object):
    ...
    def __init__(self, main_window, options):
        ...
        tk.Label(pipeline_frame, text="crYOLO finetune? (still testing)").grid(row=row, sticky=tk.W)
        ...
        def update_pipeline_control_state(*args_ignored, **kwargs_ignored):
            ...
            cryolo_fine_button.config(state=tk.NORMAL if self.use_cryolo_var.get() and not self.stop_after_ctf_var.get() else tk.DISABLED)
    ...
    def save_options(self):
        ...
        try:
            ...
            if len(warnings) == 0 or tk.messagebox.askokcancel("Warning", "\n".join(warnings), icon='warning',
    ...
    for ipass in range(first_pass, nr_passes):
        ...
        if not opts.stop_after_ctf_estimation:
            ...
            #### CRYOLO INSERT BEGIN ####
                ...
                subprocess.Popen([os.path.join(cryolo_relion_directory, 'CryoloPipeline.py'), '--num_repeats', '{}'.format(opts.preprocess_repeat_times), '--runjobs', "{}".format(runjobs), '--motioncorr_job', motioncorr_job, '--ctffind_job', ctffind_job, '--ipass', '{}'.format(ipass), '--user_opt_file', "{}".format(option_files), '--gui', '{}'.format(gui), '--manpick_job', manpick_job])
            ...
            while continue_this_pass:
                ...
                for ibatch  in range(0, nr_batches):
                        ...
                        if (ipass == 0 and opts.do_class2d) or (ipass == 1 and opts.do_class2d_pass2):
                            ...
                            if ((not already_had_it) or rerun_batch1):
                                ...
                                if opts.cryolo_finetune and opts.autopick_do_cryolo and not done_fine_tune:
                                    ...
                                    alias = 'fine_tune'
                                    subset_fine_job, already_had_it = addJob('Select', subset_fine_name, SETUP_CHECK_FILE, subset_fine_options, alias=alias)
                                    RunJobs([subset_fine_job], 1, 1, 'SUBSET')
                                    ### Fine Tuning for Cryolo
                                    fine_particles_star_file = 'Select/{}/particles.star'.format(alias)
                                    in_doc_fine = gemmi.cif.read_file(particles_star_file)
					...
					# Run in background so relion_it can carry on processing new data. Training can take a while...
                                        subprocess.Popen([os.path.join(cryolo_relion_directory, 'CryoloFineTuneJob.py'), '--in_parts', fine_particles_star_file, '--o', 'ExternalFine', '--box_size', '{}'.format(opts.extract_boxsize)])
