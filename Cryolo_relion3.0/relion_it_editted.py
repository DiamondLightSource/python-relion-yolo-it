    ...
    for ipass in range(first_pass, nr_passes):
        ...
        if not opts.stop_after_ctf_estimation:
            ...
            #### CRYOLO INSERT BEGIN ####
                ...
                subprocess.Popen(['/home/yig62234/Documents/pythonEM/Cryolo_relion3.0/RunJobsCryolo.py', '--num_repeats', '{}'.format(opts.preprocess_repeat_times), '--runjobs', "{}".format(runjobs), '--motioncorr_job', motioncorr_job, '--ctffind_job', ctffind_job, '--ipass', '{}'.format(ipass), '--user_opt_file', "{}".format(option_files)])
                            ...
                            if ((not already_had_it) or rerun_batch1):
                                ...
                                if opts.cryolo_finetune and opts.autopick_do_cryolo:
                                    ...
                                    if len(np.unique(data_as_dict['_rlnmicrographname'])) >= 8:
                                        ...
                                        cryolo_fineoptions = ['--in_parts {}'.format(particles_star_file),
                                        ...
                                        for cry_option in cryolo_fineoptions:
                                            ...
                                            option_string += ' '
                                        command = '~/Documents/pythonEM/Cryolo_relion3.0/external_cryolo_fine_3.py ' + option_string
                                        print(' RELION_IT: RUNNING {}'.format(command))

                                        subprocess.Popen(['/home/yig62234/Documents/pythonEM/Cryolo_relion3.0/external_cryolo_fine_3.py', '--in_parts', particles_star_file, '--o', 'ExternalFine', '--box_size', '{}'.format(opts.extract_boxsize)])
