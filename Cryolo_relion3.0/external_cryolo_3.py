...
def run_job(project_dir, job_dir, args_list):
    ...
    if args.in_model is None:
        os.system(f"/home/yig62234/Documents/pythonEM/Cryolo_relion3.0/qsub.sh cryolo_predict.py -c config.json -i {os.path.join(project_dir, job_dir, 'cryolo_input')} -o {os.path.join(project_dir, job_dir, 'gen_pick')} -w /dls_sw/apps/EM/crYOLO/cryo_phosaurus/gmodel_phosnet_20190516.h5 -g 0 -t {thresh}")
    else:
        print('Running from model {}'.format(model))
        os.system(f"/home/yig62234/Documents/pythonEM/Cryolo_relion3.0/qsub.sh cryolo_predict.py -c config.json -i {os.path.join(project_dir, job_dir, 'cryolo_input')} -o {os.path.join(project_dir, job_dir, 'gen_pick')} -w {model} -g 0 -t {thresh}")
    ### WAIT FOR DONEFILE! ###
