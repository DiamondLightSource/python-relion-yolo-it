...
def run_job(project_dir, job_dir, args_list):
    ...
    for micro in range(len(data_as_dict['particles']['_rlnmicrographname'])):
        ...
        try:
            print(os.path.join(project_dir, data_as_dict['particles']['_rlnmicrographname'][micro]))
            print(os.path.join(project_dir, job_dir, 'train_image', os.path.split(data_as_dict['particles']['_rlnmicrographname'][micro])[-1]))
            os.link(os.path.join(project_dir, data_as_dict['particles']['_rlnmicrographname'][micro]), os.path.join(project_dir, job_dir, 'train_image', os.path.split(data_as_dict['particles']['_rlnmicrographname'][micro])[-1]))
        except: print('{} already exists'.format(micro))

        box_name = os.path.splitext(os.path.split(data_as_dict['particles']['_rlnmicrographname'][micro])[-1])[0] + '.box'

        individual_files = open(os.path.join('train_annotation', box_name), 'a+')
        individual_files.write(f"{data_as_dict['particles']['_rlncoordinatex'][micro] - int(box_size)/2}\t")
        individual_files.write(f"{data_as_dict['particles']['_rlncoordinatey'][micro] - int(box_size)/2}\t")
