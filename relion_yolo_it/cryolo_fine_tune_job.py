...
def run_job(project_dir, job_dir, args_list):
    ...
    while not os.path.exists(particle_file):
        count += 1
        if count > 60:
            print(f' cryolo_fine_tune_job: giving up after waiting for over {count * 10} seconds for particle file {particle_file} to appear')
            raise AssertionError('Timeout waiting for input file')
        if count % 6 == 0:
            print(f' cryolo_fine_tune_job: still waiting for particle file {particle_file} to appear after {count * 10} seconds')
        time.sleep(10)
